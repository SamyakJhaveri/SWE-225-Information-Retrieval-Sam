    ...
    for index in invertedIndexDict:
        ...
        if os.path.exists(filepath):
            with open(filepath,"r") as input_file:
                old_file = os.path.join("IndexData", filename)
                if index[0].isdigit():
                    new_file = os.path.join("IndexData/numold.txt")
                else:
                    new_file = os.path.join("IndexData", index[0]+"old.txt")
            ...
            with open(filepath,"a") as input_file:
                input_file.write(json.dumps(dumpDict)+"\n")
...
def createInvertedIndex():
    invertedIndexDict=dict()
        ...
        #   }
            ...
            # }
        # }
    for curr_doc_index,name in enumerate (glob.glob('/Users/poojabhatia/Documents/Info Retrieval/A4 new/DEV/*/*')):
